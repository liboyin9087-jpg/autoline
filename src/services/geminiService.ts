...
const getOptimalTokens = (persona: AIPersona): number => {
  const tokenMap: Record<AIPersona, number> = {
    [AIPersona.CONCISE]: 512,
    [AIPersona.FRIEND]: 1024,
    [AIPersona.CONSULTANT]: 2048,
    [AIPersona.CREATIVE]: 3072,
    [AIPersona.TECH]: 4096
  };
...
export const sendMessageToGemini = async (history: Message[], mode: AppMode, userLocation: any, settings?: AppSettings) => {
  ...
  // 修正：優先使用角色定義的 Token 量，除非用戶手動設定了極端值 (例如 > 5000)
  // 這樣切換角色時才會真正改變回答長度
  const autoTokens = getOptimalTokens(persona);
  const userTokens = settings?.maxOutputTokens;
  // 如果用戶設定值是預設的 4096 或 8192，我們就使用角色的自動設定
  const optimalTokens = (userTokens === 4096 || userTokens === 8192) ? autoTokens : userTokens;
