...
const QuickActionBtn = ({ icon, label, subLabel, colorClass, onClick }: { icon: React.ReactNode, label: string, subLabel: string, colorClass: string, onClick: () => void }) => (
  <button onClick={onClick} className="relative overflow-hidden group flex flex-col items-center justify-center p-4 rounded-2xl border border-white/40 bg-white/60 backdrop-blur-md shadow-sm hover:shadow-md hover:scale-105 transition-all duration-200">
...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-fairy-bg font-sans overflow-hidden relative">
      ...
      <Header mode={mode} onModeChange={setMode} onReset={handleReset} onOpenSettings={() => setIsSettingsOpen(true)} showBackButton={true} onBack={handleReset} onSearchToggle={() => setIsSearchOpen(!isSearchOpen)} />
          ...
          <div className="flex flex-col items-center justify-center animate-fade-in mt-6 mb-8">
            <div className="w-full px-4">
              ...
              <div className="flex items-center justify-between mb-4 opacity-70">
                ...
                <button onClick={() => setIsQuickActionsManagerOpen(true)} className="text-xs text-fairy-primary hover:text-fairy-dark flex items-center gap-1 font-medium"><Edit3 size={12} /> 自訂法寶</button>
              </div>
